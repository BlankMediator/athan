...
- **Qibla:** a precise bearing from true north, with optional live Windows compass orientation when a sensor provides true north. Magnetic headings rotate the dial using a bundled World Magnetic Model correction for the saved location. The red needle points north; the separate Kaaba icon marks the Qibla relative to the device heading.
- **Athan & sounds:** per-prayer recording choices, native file selection that opens in the selected recording’s folder, full playback with pause, resume and stop, volume and dua after Athan.
...
In **Settings → Location → Use device location**, request a position from Windows, review the exact coordinates, accuracy and suggested nearby city's time zone, then choose **Use these coordinates → Save changes**. No location request is made automatically. The time zone is a nearest-city suggestion, so review it near borders. Windows privacy settings and available hardware determine whether a position can be returned; country/city and manual entry remain available. In **Qibla**, use **Use device compass** to start the sensor and **Stop device compass** to release it. Turn the top of the screen toward the Kaaba icon until the alignment message appears. It also stops when leaving Qibla or hiding the window. Stale readings revert to the calculated bearing. Keep the device flat, with the visible screen’s top edge ahead. Magnetic readings use an estimated true-north correction for the saved coordinates; move away from magnetic objects and calibrate the device if readings drift. Athan does not change Windows privacy settings.
...
**Hadith library** opens collections directly from the server when selected; no installation or API key is needed. The **Save everything offline** button in the top toolbar saves all 17 collections, alongside the app, locations, readings and default recordings. Progress is shared across pages, cancellation preserves completed collections, and retry resumes the remaining work. Desktop and native mobile apps read their included collection files without needing a separate download. Source wording, grades, references, provenance and partial-coverage notices remain available. See [dataset sources and limitations](docs/hadith-data-sources.md).

Use the three-line **menu** button to collapse or expand navigation on desktop and phones; the choice is remembered. Playing a recording shows an audio player above the bottom edge on every page, with **Pause audio**, **Resume audio** and **Dismiss / stop sound**. Manual playback plays the complete recording within the configured playback limit. **Enable Athan** unlocks browser audio for scheduled calls; the page must remain open and the device awake. Use **Athan & sounds → Use default recordings → Save changes** to restore the included voices if an older profile has silent choices.
