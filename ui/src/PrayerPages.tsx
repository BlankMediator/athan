...
export function QiblaPage() {
  const device = useDeviceCompass();
  const { snapshot: s, navigate } = useApp(), location = s.config.locations.find(l => l.id === s.config.activeLocation)!;
  return <div className="qibla-layout"><section className="compass-panel panel"><span className="eyebrow">A SHARED DIRECTION</span><Compass bearing={s.day.qibla} heading={device.heading} /><div className="bearing-value">{s.day.qibla.toFixed(1)}<span>°</span></div><p>Clockwise from true north</p><span className="compass-rule" />{device.controls}</section><div className="qibla-information"><div className="qibla-quote"><span className="quote-ornament">✦</span><h2>Wherever you are,<br /><em>find your connection.</em></h2><p>Your direction to the Kaaba, calculated from your saved location.</p></div><Panel><span className="eyebrow">YOUR STARTING POINT</span><div className="place-detail"><MapPin size={22} /><div><h3>{location.name}</h3><p>{location.country}</p></div></div><div className="coordinate-pair"><div><small>LATITUDE</small><strong>{Math.abs(location.latitude).toFixed(4)}° {location.latitude < 0 ? 'S' : 'N'}</strong></div><div><small>LONGITUDE</small><strong>{Math.abs(location.longitude).toFixed(4)}° {location.longitude < 0 ? 'W' : 'E'}</strong></div></div><button className="text-button" onClick={() => navigate('settings')}>Change location <ArrowUpRight size={15} /></button></Panel><p className="compass-instruction"><CompassIcon size={19} />The bearing uses your saved location. Connect the device compass for live orientation when a true-north reading is available, or align an external compass to true north.</p></div></div>;
}
