...
import { ArrowUpRight, BookOpen, Bell, CalendarDays, ChevronRight, Compass as CompassIcon, Globe2, LayoutDashboard, LoaderCircle, MapPin, Minus, Moon, Settings2, Square, Sun, Volume2, X, Activity, Check, AlertCircle, Pause, Play } from 'lucide-react';
...
export function App() {
  ...
  const refreshing = useRef(false), request = useRef(0);
  ...
  return <div className="desktop-shell">
    ...
    {snapshot && (snapshot.runtime.alerts.length > 0 || snapshot.runtime.playing) && <div className="active-alert" role="status">
      <span className="active-alert-icon"><Volume2 size={22} /></span>
      <div><strong>{snapshot.runtime.alerts.at(-1)?.title ?? 'Sound preview'}{snapshot.runtime.alerts.length > 1 ? ` + ${snapshot.runtime.alerts.length - 1} more` : ''}</strong><small>{snapshot.runtime.alerts.length ? 'Future prayers stay enabled.' : 'Stop this preview.'}</small></div>
      <button className="button primary" onClick={() => void act(() => window.athan.dismissAlerts())}><Square size={14} fill="currentColor" />Dismiss / stop sound</button>
    </div>}
